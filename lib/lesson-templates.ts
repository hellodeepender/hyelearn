  ...
  for (let i = 0; i < Math.min(3, words.length); i++) {
    ...
    results.push({
      ...
      exercise_data: {
        ...
        explanation_en: `${d.armenian} (${transliterate(d.armenian as string)}) means "${d.english}"`,
        sort_order: sort.val,
      },
      sort_order: sort.val++,
    });
  ...
  if (words.length >= 2) {
    ...
    results.push({
      ...
      exercise_data: {
        ...
        explanation_hy: `${fb.armenian} = ${fb.english}`, explanation_en: `${fb.armenian} (${transliterate(fb.armenian as string)}) means ${fb.english}`,
        sort_order: sort.val,
      },
      sort_order: sort.val,
    });
  ...
  for (const item of fbItems) {
    ...
    results.push({
      ...
      exercise_data: {
        ...
        explanation_en: isLetter ? `${d.transliteration} sounds like "${d.sound}"` : `${d.armenian} (${transliterate(d.armenian as string)}) means ${d.english}`,
        sort_order: sortVal,
      },
      sort_order: sortVal++,
    });
